...
$key = Get-UninstallRegistryKey -SoftwareName 'LingoGeek*'
...
elseif ($key.Count -eq 0) {
  Write-Warning 'LingoGeek is not installed, or was installed outside of Chocolatey. Nothing to do.'
}
else {
  Write-Warning "$($key.Count) matches found for LingoGeek. Remove it manually rather than guessing:"
